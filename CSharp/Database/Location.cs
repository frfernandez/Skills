    ...
    public class Location : Connection
    {
        ...
        public string Schema(CommandType ObjectType, string Object)
        {
            ...
            if (ObjectType == CommandType.StoredProcedure)
                SQLCommand.CommandText = "Sistema.FltEsquemaProcedimento";
            else if (ObjectType == CommandType.Text)
                SQLCommand.CommandText = "Sistema.FltEsquemaTabela";

            SQLCommand.Parameters.Clear();
            SqlCommandBuilder.DeriveParameters(SQLCommand);

            SQLCommand.Parameters["@Objeto"].Value = Object;

            SqlDataReader Leitor = SQLCommand.ExecuteReader();
            if (Leitor.Read())
                Result = Leitor["Esquema"].ToString();
            Leitor.Close();
            Leitor.Dispose();
